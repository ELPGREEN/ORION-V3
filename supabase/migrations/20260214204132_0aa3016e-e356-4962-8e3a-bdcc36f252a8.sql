...
-- Increase statement timeout for hybrid search functions to handle 63k+ embeddings
CREATE OR REPLACE FUNCTION public.hybrid_search_legal_v3(query_embedding vector, query_text text, match_count integer DEFAULT 20, semantic_weight double precision DEFAULT 0.55, keyword_weight double precision DEFAULT 0.25, authority_weight double precision DEFAULT 0.10, recency_weight double precision DEFAULT 0.10, filter_source text DEFAULT NULL::text, filter_type text DEFAULT NULL::text, filter_sources text[] DEFAULT NULL::text[], filter_date_from text DEFAULT NULL::text, filter_date_to text DEFAULT NULL::text)
 RETURNS TABLE(id uuid, title text, content text, source text, source_label text, content_type text, url text, published_date text, metadata jsonb, semantic_score double precision, keyword_score double precision, authority_score double precision, recency_score double precision, combined_score double precision)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Also increase v2 and v1 timeouts
ALTER FUNCTION public.hybrid_search_legal_v2 SET statement_timeout = '30s';
ALTER FUNCTION public.hybrid_search_legal SET statement_timeout = '30s';
