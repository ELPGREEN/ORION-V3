...
-- Add statement_timeout to hybrid_search_legal_v3 to prevent long-running queries
...
-- Also add statement_timeout to v2 and v1 fallback functions
CREATE OR REPLACE FUNCTION public.hybrid_search_legal_v2(
  query_embedding vector, query_text text, match_count integer DEFAULT 15, 
  semantic_weight double precision DEFAULT 0.6, keyword_weight double precision DEFAULT 0.25, 
  authority_weight double precision DEFAULT 0.15, filter_source text DEFAULT NULL::text, 
  filter_type text DEFAULT NULL::text
)
RETURNS TABLE(
  id uuid, title text, content text, source text, source_label text, 
...
CREATE OR REPLACE FUNCTION public.hybrid_search_legal(
  query_embedding vector, query_text text, match_count integer DEFAULT 10, 
  semantic_weight double precision DEFAULT 0.7, keyword_weight double precision DEFAULT 0.3, 
  filter_source text DEFAULT NULL::text, filter_type text DEFAULT NULL::text
)
RETURNS TABLE(
  id uuid, title text, content text, source text, source_label text, 
  content_type text, url text, published_date text, metadata jsonb, 
...
BEGIN
  ...
  ORDER BY combined_score DESC LIMIT LEAST(match_count, 30);
END; $function$;

-- Also add timeout to search_neural_knowledge
...
RETURNS TABLE(
  id uuid, title text, content text, source_type text, source_reference text, 
  tags text[], semantic_score double precision, keyword_score double precision, 
  combined_score double precision
)
LANGUAGE plpgsql
SECURITY DEFINER
