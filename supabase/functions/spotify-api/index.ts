...
async function spotifyApi(
  ...
  try {
    ...
    console.error(
      `Spotify API non-JSON (${res.status}):`,
  ...
  try {
    ...
    if (action === "search" || action === "available_genres") {
      ...
      try {
        ...
        console.error(
          "Spotify token response not JSON:",
          tokenText.substring(0, 200),
        );
        return err(
          `Spotify auth failed (status ${tokenRes.status}). Check SPOTIFY_CLIENT_ID/SECRET.`,
          502,
        );
      }
      if (!tokenData.access_token) {
        return err(`Spotify auth error: ${tokenData.error || "no token"}`, 502);
      }
    ...
    if (action === "status") {
      ...
      return json({
        ...
        can_sdk: scopes.includes("streaming"),
      });
    }
    ...
    if (action === "sdk_token") {
      ...
      if (!canSdk) {
        return json({
          access_token: null,
          can_sdk: false,
          reason: "missing_streaming_scope",
        });
      }

      const accessToken = await getValidToken(userId);
      if (!accessToken) {
        return err("Spotify not connected or token expired", 401);
      }
    ...
    if (action === "disconnect") {
      const sb = getSupabase();
      await sb.from("user_integration_tokens").delete().eq("user_id", userId)
        .eq("provider", "spotify");
      return json({ success: true });
    }
    ...
    const actionMap: Record<string, () => Promise<unknown>> = {
      ...
      recently_played: () =>
        spotifyApi(
          token,
          `/me/player/recently-played?limit=${body.limit || 20}`,
        ),
      get_user_profile: () => spotifyApi(token, "/me"),
      get_playlists: () =>
        spotifyApi(token, `/me/playlists?limit=${body.limit || 50}`),
      playback_state: () => spotifyApi(token, "/me/player"),
      artist_top_tracks: () =>
        spotifyApi(token, `/artists/${body.artist_id}/top-tracks?market=BR`),
      track_features: () =>
        spotifyApi(token, `/audio-features/${body.track_id}`),
      recommendations: async () => {
        const params = new URLSearchParams();
        if (body.seed_artists) {
          params.set("seed_artists", body.seed_artists as string);
        }
        if (body.seed_tracks) {
          params.set("seed_tracks", body.seed_tracks as string);
        }
        if (body.seed_genres) {
          params.set("seed_genres", body.seed_genres as string);
        }
        if (body.limit) params.set("limit", String(body.limit));
        if (body.target_energy) {
          params.set("target_energy", body.target_energy as string);
        }
        if (body.target_valence) {
          params.set("target_valence", body.target_valence as string);
        }
        if (body.target_tempo) {
          params.set("target_tempo", body.target_tempo as string);
        }
      ...
      play: () =>
        spotifyApi(
          ...
          "/me/player/play",
          "PUT",
          body.uris || body.context_uri
            ? { uris: body.uris, context_uri: body.context_uri }
            : undefined,
        ),
      ...
      volume: () =>
        spotifyApi(
          token,
          `/me/player/volume?volume_percent=${body.volume}`,
          "PUT",
        ),
      create_playlist: () =>
        spotifyApi(token, `/users/${body.user_id}/playlists`, "POST", {
          name: body.name,
          description: body.description,
          public: body.is_public,
        }),
      add_to_playlist: () =>
        spotifyApi(token, `/playlists/${body.playlist_id}/tracks`, "POST", {
          uris: body.uris,
        }),
      get_playlist_tracks: () =>
        spotifyApi(
          token,
          `/playlists/${body.playlist_id}/tracks?limit=${body.limit || 50}`,
        ),
    };
