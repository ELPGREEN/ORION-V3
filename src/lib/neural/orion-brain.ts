...
export async function orionBrain(request: OrionRequest): Promise<OrionResponse> {
  ...
  // BOLT V2.0 - Unified Consciousness Gateway
  // Non-streaming requests route through the Pentagon Orchestrator
  try {
    ...
    return {
      ...
      sector: detectSector(input),
      agentUsed: result.metadata.earlyExit ? `pentagon-fast-${result.metadata.earlyExitType}` : "pentagon-pizza-v1",
    };
  } catch (error) {
    console.error("[ORION-BRAIN] Pentagon bypass due to error:", error);
