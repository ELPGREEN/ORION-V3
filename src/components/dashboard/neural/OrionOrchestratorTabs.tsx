/**
 * Orchestrator Tabs — merged from OrionOrchestratorPage
 * Contains Operacional, Financeiro, Comercial KPIs
 */
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const chartData = Object.entries(dailyMap).sort().slice(-14).map(([day, v]) => ({
        day: day.slice(5), tarefas: v.total, erros: v.errors,
      }));
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="border-border/50">
        <CardHeader className="pb-2"><CardTitle className="text-sm">Fluxo de Tarefas — Últimos 14 Dias</CardTitle></CardHeader>
        <CardContent>
  ...
  const addEntry = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("orion_financial_entries").insert({
        user_id: user.id, type: formData.type, category: formData.category,
        description: formData.description, amount_cents: Math.round(parseFloat(formData.amount) * 100), date: formData.date,
      });
      if (error) throw error;
    },
    onSuccess: () => { toast.success("Entrada registrada"); setShowForm(false); setFormData({ type: "saida", category: "operacional", description: "", amount: "", date: new Date().toISOString().slice(0, 10) }); refetch(); },
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 flex-wrap">
        <Button variant="outline" size="sm" onClick={() => setShowForm(!showForm)}><Plus className="h-3 w-3 mr-1" /> Entrada Manual</Button>
        <Button variant="outline" size="sm" onClick={() => projectionsMutation.mutate()} disabled={projectionsMutation.isPending}>
          {projectionsMutation.isPending ? <Loader2 className="h-3 w-3 mr-1 animate-spin" /> : <Brain className="h-3 w-3 mr-1" />} Projeções IA
        </Button>
        <Button variant="ghost" size="sm" onClick={() => refetch()}><RefreshCw className="h-3 w-3" /></Button>
      </div>
      {showForm && (
        <Card className="border-border/50">
          <CardContent className="p-4 space-y-3">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ...
              <select className="rounded-md border border-input bg-background px-3 py-2 text-sm" value={formData.category} onChange={e => setFormData(p => ({ ...p, category: e.target.value }))}>
                <option value="operacional">Operacional</option><option value="marketing">Marketing</option><option value="salarios">Salários</option>
                <option value="impostos">Impostos</option><option value="investimento">Investimento</option><option value="outros">Outros</option>
              </select>
            ...
            <Button size="sm" onClick={() => addEntry.mutate()} disabled={addEntry.isPending || !formData.amount}>
              {addEntry.isPending ? <Loader2 className="h-3 w-3 animate-spin mr-1" /> : null} Salvar
            </Button>
          </CardContent>
        </Card>
      )}
...
export default function OrionOrchestratorTabs() {
  return (
    <div className="space-y-4">
      <AlertsSection />
