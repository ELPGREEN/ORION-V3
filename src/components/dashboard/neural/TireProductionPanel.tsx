/**
 * Tire Production Line Panel — Real-time OEE, inspection & conveyor control
 * Connects to /tire_line/* ROS2 services via ROSBridge
 */
...
export default function TireProductionPanel() {
  ...
  const { isConnected, subscribe, callService } = useRosBridge();
  ...
  const handleConveyor = useCallback(async (start: boolean) => {
    try {
      ...
      toast.error("Falha ao controlar esteira");
    }
  }, [callService]);
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <Card>
          <CardContent className="pt-4 pb-3 text-center">
            <Gauge className={`h-5 w-5 mx-auto mb-1 ${oeeColor}`} />
            <p className="text-[10px] text-muted-foreground uppercase">OEE</p>
            <p className={`text-2xl font-bold font-mono ${oeeColor}`}>{oee.toFixed(1)}%</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-3 text-center">
            <BarChart3 className="h-5 w-5 mx-auto mb-1 text-primary" />
            <p className="text-[10px] text-muted-foreground uppercase">Produzidos</p>
            <p className="text-2xl font-bold font-mono">{stats?.tire_count ?? lineStatus?.tire_count ?? 0}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-3 text-center">
            ...
            <p className="text-2xl font-bold font-mono text-green-500">{stats?.good_count ?? lineStatus?.good_count ?? 0}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-3 text-center">
            <XCircle className="h-5 w-5 mx-auto mb-1 text-destructive" />
            <p className="text-[10px] text-muted-foreground uppercase">Defeitos</p>
            <p className="text-2xl font-bold font-mono text-destructive">{stats?.defect_count ?? lineStatus?.defect_count ?? 0}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-3 text-center">
            <Zap className="h-5 w-5 mx-auto mb-1 text-blue-400" />
            <p className="text-[10px] text-muted-foreground uppercase">Pneus/hora</p>
            <p className="text-2xl font-bold font-mono">{(stats?.throughput_per_hour ?? lineStatus?.throughput_per_hour ?? 0).toFixed(0)}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Badge variant={conveyorRunning ? "default" : "secondary"}>
                {lineStatus?.conveyor_speed_mps?.toFixed(2) ?? "0.00"} m/s
              </Badge>
            </div>
            {lineStatus?.uptime_hours !== undefined && (
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Timer className="h-3 w-3" />
                Uptime: {lineStatus.uptime_hours.toFixed(1)}h
              </div>
            )}
            <div className="flex gap-2">
              <Button size="sm" className="flex-1" onClick={() => handleConveyor(true)} disabled={!isConnected || conveyorRunning}>
                <Play className="h-3.5 w-3.5 mr-1" /> Iniciar
              </Button>
              <Button size="sm" variant="destructive" className="flex-1" onClick={() => handleConveyor(false)} disabled={!isConnected || !conveyorRunning}>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">Taxa de defeito</span>
              <span className="font-mono font-medium">{(lineStatus?.defect_rate_pct ?? 0).toFixed(1)}%</span>
            </div>
            <Progress value={100 - (lineStatus?.defect_rate_pct ?? 0)} className="h-2" />
            <div className="flex gap-2">
              <Button size="sm" className="flex-1" onClick={handleInspection} disabled={!isConnected || inspecting}>
                <ScanSearch className="h-3.5 w-3.5 mr-1" />
                {inspecting ? "Inspecionando..." : "Inspecionar Pneu"}
              </Button>
              <Button size="sm" variant="outline" onClick={handleCalibrate} disabled={!isConnected || calibrating}>
