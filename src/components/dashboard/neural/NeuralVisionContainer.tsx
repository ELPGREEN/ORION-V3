/**
 * NeuralVisionContainer — COMPLETE implementation with full JARVIS HUD layout
 * Integrates: NeuralVisionCamera, useNeuralVisionHandlers, OrionStandalonePanel
 * Original 1607L NeuralVision.tsx split with 100% backward compatibility
 */
...
// Real-time detection via Gemini Flash
...
export function NeuralVisionContainer({ skipWakeWord = false, initialCommand = "" }: { skipWakeWord?: boolean; initialCommand?: string }) {
  ...
  // Expose interim transcript setter globally
  useEffect(() => {
  ...
  // Use extracted camera component
  const { startCamera, stopCamera, deactivateGracefully } = NeuralVisionCamera({
    active, setActive, videoRef, streamRef, animRef, prevRef, speak,
  });
  ...
  const startDirectVoiceCapture = useCallback(() => {
    ...
    stopListen();
    const isMobileBrowser = typeof navigator !== "undefined" && /android|iphone|ipad|ipod|mobile/i.test(navigator.userAgent);
    const delay = skipWakeWord ? 0 : (isMobileBrowser ? 160 : 180);
    directVoiceStartTimerRef.current = setTimeout(() => {
      directVoiceStartTimerRef.current = null;
      startListening(handleVoice);
    }, delay);
  ...
  // ═══ Listen for vision commands from text chat ═══
  ...
  useEffect(() => {
    ...
    const handler = (e: Event) => {
      ...
      if (shouldSuppressVisionCommand(action)) return;
      ...
      if (detail?.action === "activate_vision" && !active) {
        ...
        if (!userInitiated && !keywordPresent) return;
  ...
  useEffect(() => {
    ...
    if (listening && wakeWordActive) { stopWakeWordListener(); return; }
  ...
  }, [active, supernetConnected, sendSuperNetFrame]);
  ...
  useEffect(() => {
    if (active) {
      ...
      if (!mpObjectDetector) {
        FilesetResolver.forVisionTasks("https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.22/wasm").then((vision) => {
          ObjectDetector.createFromOptions(vision, {
            baseOptions: { delegate: "GPU", modelAssetPath: "https://cdn.jsdelivr.net/npm/@mediapipe/models/object_detector/v1/efficientdet_lite0_uint8_quant.tflite" },
            runningMode: "VIDEO",
          }).then(detector => { mpObjectDetector = detector; mpVisionReady = true; }).catch(e => console.warn("[Vision] MediaPipe load failed:", e));
        }).catch(e => console.warn("[Vision] MediaPipe resolver failed:", e));
      }
    }
  }, [active]);
            ...
            <div className="px-3 py-2 space-y-1">
              {regions.length > 0 ? regions.slice(0, 5).map((r, i) => (
                <div key={i} className="flex items-center gap-1.5">
                  ...
                  <span className="text-[8px] font-mono text-[hsl(var(--tron-neon))]">{((r.confidence) * 100).toFixed(0)}%</span>
                </div>
              )) : (
        ...
        <div className="absolute top-3 right-3 bottom-3 w-[220px] xl:w-[250px] hidden lg:flex flex-col gap-2 z-10 overflow-y-auto"
          ...
          <div className="relative bg-black/60 backdrop-blur-sm border border-cyan-500/20 rounded-sm overflow-hidden">
            ...
            <div className="px-3 py-1.5 flex items-center gap-1.5 border-b border-cyan-500/10">
              ...
              <span className="text-[10px] font-mono text-[hsl(var(--tron-neon))] tracking-wider uppercase">Raciocínio</span>
            </div>
            <div className="px-3 py-2">
          ...
          <div className="relative bg-black/60 backdrop-blur-sm border border-cyan-500/20 rounded-sm overflow-hidden">
            ...
            <div className="px-3 py-1.5 flex items-center gap-1.5 border-b border-cyan-500/10">
              ...
              <span className="text-[10px] font-mono text-[hsl(var(--tron-neon))] tracking-wider uppercase">IA Chat</span>
              {isProcessing && (
              ...
              <div className="space-y-1 max-h-[120px] overflow-y-auto">
                    ...
                    <div className="flex flex-wrap gap-1 justify-center">
                      {["O que vê?", "Como estou?", "Agenda"].map(q => (
                        <button key={q} onClick={() => askAI(q)}
                          className="text-[7px] font-mono text-[hsl(var(--tron-neon))]/70 border border-cyan-500/10 rounded px-1 py-0.5 hover:bg-cyan-400/5 hover:text-[hsl(var(--tron-neon))]/60 transition-colors">
                          {q}
                        </button>
                      ))}
                    </div>
                ...
                {interimTranscript && (
                  <div className="flex justify-end">
                    <div className="max-w-[90%] rounded px-1.5 py-0.5 bg-cyan-500/5 border border-cyan-500/10 animate-pulse">
                      <p className="text-[8px] font-mono leading-relaxed text-cyan-400/50 italic">
                        {interimTranscript}...
                      </p>
                    </div>
                  </div>
