...
## 2026-07-03 - [Neural Hot-Path & Text Utilities (BOLT V2.0)]
**Baseline:**
- computeFreeEnergy: 0.1532ms
- analyzeSemantics: 0.0174ms

**Nova Métrica:**
- computeFreeEnergy: 0.1404ms
- analyzeSemantics: 0.0164ms

**Delta (Δ):**
- computeFreeEnergy: ~8.3%
- analyzeSemantics: ~5.7%

**Learning:** Unified zero-allocation text utilities (`countWords`, `getTokensEfficiently`) reduce heap churn and latency across multiple neural modules. Consolidating search phrases into single hoisted RegExps is significantly more efficient than iterative `.includes()` or `.filter()` calls. Fixed ESLint surrogate pair issues by standardizing on escaped Unicode sequences in `MARKDOWN_CLEANUP_REGEX`.
